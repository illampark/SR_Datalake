...
if [ "$DOCKER_OK" -eq 1 ]; then
  # 환경 간 동일성은 빌드 시각(UTC)으로 판정한다. 이미지 ID 는 호스트 안에서만
  # 유효하다 — docker save/load 를 거치면 데몬이 재구성해 ID 와 SIZE 가 바뀐다.
  # 반면 이미지 config 의 created 는 보존되므로 비교 가능하다.
  # 컨테이너 생성 시각과도 혼동하지 말 것. 이미지를 반입해도 컨테이너는 새로 만들어진다.
  # DGX 는 aarch64 라 별도 빌드이므로 시각이 다른 것이 정상 — 커밋 해시로 추적한다.
