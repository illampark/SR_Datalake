"""
시나리오 8: OPC-DA → 단위 변환 체인 → Enrich → 외부 File
[OPC-DA Source] → Normalize → Unit Convert → Enrich → [External File Sink]
...
class TestS8UnitConvert:
    ...
    def _make_opcda_msg(self, value, tag="Tank.Level", unit="mm"):
        return make_message(
            tag_name=tag, value=value, unit=unit,
            connector_type="opcda", connector_id=1,
        )
    ...
    def test_custom_factor_offset(self):
        ...
        msg = self._make_opcda_msg(100, unit="custom_a")
    ...
    def test_same_unit_no_change(self):
        """동일 단위 → 변환 없음"""
        msg = self._make_opcda_msg(50, unit="celsius")
